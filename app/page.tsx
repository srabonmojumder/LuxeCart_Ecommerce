...
function DealCountdown({ endDate }: { endDate: Date }) {
    ...
    return (
        <div className="flex flex-col">
            <span className="text-[10px] text-secondary dark:text-gray-400 font-black tracking-widest uppercase mb-2.5">Offer ends in</span>
            <div className="flex gap-2.5">
                {units.map((u) => (
                    <div key={u.l} className="flex flex-col items-center">
                        <div className="w-12 h-14 rounded-xl bg-white dark:bg-slate-800 border border-primary/5 dark:border-slate-700 shadow-sm flex items-center justify-center">
                            <span className="text-2xl font-black text-accent tabular-nums">{String(u.v).padStart(2, '0')}</span>
                        </div>
                        <span className="text-[8px] font-black text-gray-400 uppercase tracking-[0.15em] mt-1.5">{u.l}</span>
                    </div>
                ))}
            </div>
        </div>
...
export default function Home() {
    ...
    return (
        <div className="bg-white dark:bg-slate-950 min-h-screen">
                ...
                <div className="relative overflow-hidden rounded-[2rem] md:rounded-[3rem] bg-gradient-to-br from-accent-900 via-accent-800 to-accent-950 px-6 py-14 md:py-20">
                    ...
                    <div className="relative max-w-7xl mx-auto">
                        <div className="text-center mb-12 md:mb-14 space-y-3">
                            <span className="inline-flex items-center gap-2 text-accent-300 font-black tracking-[0.3em] text-[10px] md:text-xs uppercase">
                                <Sparkles className="w-3.5 h-3.5" /> Trusted Worldwide
                            </span>
                            <h2 className="text-3xl md:text-5xl font-black text-white tracking-tighter">The Numbers Speak</h2>
                        </div>
                            ...
                            {statItems.map((s, i) => (
                                ...
                                >
                                    ...
                                    <AnimatedCounter
                                        ...
                                        suffix={s.suffix}
                                        className="block text-4xl md:text-6xl font-black tracking-tighter bg-gradient-to-b from-white to-accent-200 bg-clip-text text-transparent"
                                    />
                ...
                <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
                    {features.map((f, i) => (
                        <div key={i} className="flex flex-col md:flex-row items-center gap-3 md:gap-4 group justify-center md:justify-start text-center md:text-left">
                            <div className="w-12 h-12 rounded-2xl bg-gray-50 dark:bg-slate-800 flex items-center justify-center group-hover:bg-accent group-hover:text-white transition-colors flex-shrink-0">
                ...
                <section className="py-12 md:py-24">
                    <div className="max-w-7xl mx-auto px-4">
                        <div className="text-center max-w-xl mx-auto mb-16 space-y-4">
                            <span className="text-accent font-black tracking-[0.3em] text-xs uppercase">Curated For You</span>
                            <h2 className="text-4xl md:text-6xl font-black text-primary dark:text-white tracking-tighter">Shop by Collection</h2>
                        </div>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
                                            ...
                                            <h3 className="text-white text-2xl md:text-3xl font-black tracking-tighter mb-3">{cat.name}</h3>
                                            <span className="inline-flex items-center gap-2 text-white text-xs font-black tracking-widest uppercase group-hover:gap-3 transition-all">
            ...
            <section className="py-12 md:py-24 bg-gray-50 dark:bg-slate-900/50">
                <div className="max-w-7xl mx-auto px-4">
                    <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
                        <div className="max-w-xl space-y-4">
                            <span className="inline-flex items-center gap-2 text-accent font-black tracking-[0.3em] text-xs uppercase">
                                <Sparkles className="w-4 h-4" /> Just Landed
                            </span>
                            <h2 className="text-4xl md:text-6xl font-black text-primary dark:text-white tracking-tighter">New Arrivals</h2>
                        </div>
                        <Link href="/products?sort=newest" className="text-sm font-black tracking-widest uppercase text-accent hover:underline">View All →</Link>
                    </div>
                    ...
                    <Link href={dealProduct ? `/products/${dealProduct.slug}` : '/products?filter=sale'} className="relative aspect-[4/5] md:aspect-square rounded-[2rem] overflow-hidden shadow-2xl group block">
                        ...
                        {dealProduct && (dealProduct.discount ?? 0) > 0 && (
                            <div className="absolute top-8 left-8">
                                <span className="bg-hot text-white px-4 py-2 rounded-xl text-sm font-black tracking-widest uppercase shadow-lg">Save {dealProduct.discount}%</span>
                            </div>
                        )}
                    </Link>
                        ...
                        <div className="flex flex-wrap gap-8 items-center">
                            {dealProduct ? (
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-4xl md:text-5xl font-black text-primary dark:text-white">${dealNow.toFixed(2)}</span>
                                </div>
                            ) : (
                                <div className="flex flex-col">
                                    <span className="text-4xl md:text-5xl font-black text-primary dark:text-white">Up to 50% Off</span>
                                </div>
                            )}
                            <div className="h-16 w-[1px] bg-gray-200 dark:bg-slate-700 hidden sm:block" />
                            <DealCountdown endDate={promoEnd} />
                        </div>
                        <Link href={dealProduct ? `/products/${dealProduct.slug}` : '/products?filter=sale'} className="inline-flex items-center gap-4 bg-primary dark:bg-accent text-white px-10 py-5 rounded-2xl font-black text-sm tracking-[0.2em] hover:bg-black dark:hover:bg-accent-600 hover:scale-105 active:scale-95 transition-all shadow-xl uppercase">
            ...
            {testimonials.length > 0 && (
                <section className="py-16 md:py-28 bg-gray-50 dark:bg-slate-900/40">
                    <div className="max-w-7xl mx-auto px-4">
                        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-14">
                            <div className="space-y-4 max-w-xl">
                                <span className="text-accent font-black tracking-[0.3em] text-xs uppercase">Loved by Customers</span>
                                <h2 className="text-4xl md:text-6xl font-black text-primary dark:text-white tracking-tighter">What People Say</h2>
                            </div>
                            {stats && stats.reviewCount > 0 && (
                                <div className="flex items-center gap-4 bg-white dark:bg-slate-900 border border-primary/5 dark:border-slate-800 rounded-2xl px-6 py-4 shadow-sm self-start lg:self-auto">
                                    <span className="text-5xl font-black text-primary dark:text-white tracking-tighter leading-none">{stats.avgRating.toFixed(1)}</span>
