                ...
                : reviews.length === 0 ? <p className="text-secondary dark:text-gray-400">No reviews yet.</p>
                    : (
                        <div className="space-y-3">
                            {reviews.map((r) => (
                                <div key={r.id} className="flex items-start justify-between gap-4 p-5 bg-white dark:bg-slate-900 border border-primary/5 dark:border-slate-800 rounded-2xl">
                                    <div className="min-w-0 space-y-1.5">
                                        <div className="flex items-center gap-3 flex-wrap">
                                            ...
                                            <span className="text-sm font-bold text-primary dark:text-white">{r.product}</span>
                                            <span className="text-xs text-gray-400">by {r.author}</span>
                                        </div>
                                        {r.comment && <p className="text-sm text-secondary dark:text-gray-400">{r.comment}</p>}
                                    </div>
