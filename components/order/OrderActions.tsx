...
export default function OrderActions({ order, onChanged }: { order: Order; onChanged: () => void }) {
    ...
    return (
        <div className="bg-white dark:bg-ink-900 border border-primary/5 dark:border-slate-800 rounded-2xl p-6 space-y-4">
            <h2 className="font-medium text-primary dark:text-white uppercase tracking-tight">Actions</h2>
            ...
            <div className="flex flex-wrap gap-3">
                <Link
                    ...
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-primary text-white dark:bg-ink-800 text-xs font-medium uppercase tracking-widest hover:opacity-90"
                >
                ...
                {order.canCancel && (
                    <button
                        ...
                        className="inline-flex items-center gap-2 px-5 py-3 rounded-xl border-2 border-hot/40 text-hot text-xs font-medium uppercase tracking-widest hover:bg-hot/5"
                    >
                ...
                {canReturn && (
                    <button
                        ...
                        className="inline-flex items-center gap-2 px-5 py-3 rounded-xl border-2 border-accent/40 text-accent text-xs font-medium uppercase tracking-widest hover:bg-accent/5"
                    >
            ...
            {showCancel && (
                <div className="p-4 rounded-xl bg-hot/5 border border-hot/20 space-y-3">
                    ...
                    <textarea
                        ...
                        rows={2}
                        className="w-full px-3 py-2 rounded-lg bg-white dark:bg-ink-800 border border-primary/10 dark:border-slate-700 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-hot/40"
                    />
                    <div className="flex gap-2">
                        <button onClick={doCancel} disabled={busy === 'cancel'} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-hot text-white text-xs font-medium uppercase tracking-widest disabled:opacity-60">
                            {busy === 'cancel' && <Loader2 className="w-4 h-4 animate-spin" />} Confirm cancel
                        </button>
                        <button onClick={() => setShowCancel(false)} className="px-5 py-2.5 rounded-lg text-xs font-medium uppercase tracking-widest text-secondary dark:text-gray-400">Keep order</button>
                    </div>
                </div>
            )}
            ...
            {showReturn && (
                <div className="p-4 rounded-xl bg-accent/5 border border-accent/20 space-y-3">
                    ...
                    <textarea
                        ...
                        rows={3}
                        className="w-full px-3 py-2 rounded-lg bg-white dark:bg-ink-800 border border-primary/10 dark:border-slate-700 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-accent/40"
                    />
                    <div className="flex gap-2">
                        <button onClick={doReturn} disabled={busy === 'return'} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-accent text-white text-xs font-medium uppercase tracking-widest disabled:opacity-60">
                            {busy === 'return' && <Loader2 className="w-4 h-4 animate-spin" />} Submit request
                        </button>
                        <button onClick={() => setShowReturn(false)} className="px-5 py-2.5 rounded-lg text-xs font-medium uppercase tracking-widest text-secondary dark:text-gray-400">Cancel</button>
                    </div>
                </div>
            )}
        </div>
