...
export default function ProfileForm() {
    ...
    const field = 'w-full px-4 py-3 bg-gray-50 dark:bg-ink-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-accent text-gray-900 dark:text-white disabled:opacity-60';

    return (
        <form onSubmit={save} className="grid md:grid-cols-2 gap-6 max-w-2xl">
            <div className="md:col-span-2 space-y-2">
                <label className="text-[10px] font-medium uppercase tracking-[0.3em] text-gray-400">Display Name</label>
                <input className={field} value={displayName} onChange={(e) => setDisplayName(e.target.value)} required />
            </div>
            <div className="md:col-span-2 space-y-2">
                <label className="text-[10px] font-medium uppercase tracking-[0.3em] text-gray-400">Email (read-only)</label>
                <input className={field} value={user?.email ?? ''} disabled />
            </div>
            <div className="md:col-span-2 space-y-2">
                <label className="text-[10px] font-medium uppercase tracking-[0.3em] text-gray-400">Photo URL (optional)</label>
