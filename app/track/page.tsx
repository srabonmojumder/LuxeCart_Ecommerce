    ...
    return (
        <div className="max-w-3xl mx-auto px-4 md:px-8 py-10 pb-24 space-y-8">
            <header className="text-center space-y-2">
                <span className="text-accent font-medium tracking-[0.3em] text-xs uppercase">Order Tracking</span>
                <h1 className="text-3xl md:text-5xl font-medium text-primary dark:text-white tracking-tight">Track Your Order</h1>
                <p className="text-secondary dark:text-gray-400">Enter your order number and the email you used at checkout.</p>
            </header>

            <form onSubmit={(e) => { e.preventDefault(); lookup(orderNo, email); }} className="bg-white dark:bg-ink-900 border border-primary/5 dark:border-slate-800 rounded-2xl p-6 grid sm:grid-cols-[1fr_1fr_auto] gap-3 items-end">
                <div className="space-y-1.5">
                    <label className="text-[10px] font-medium uppercase tracking-widest text-gray-400">Order Number</label>
                    <input value={orderNo} onChange={(e) => setOrderNo(e.target.value)} required placeholder="LC-0010"
                        className="w-full px-4 py-3 bg-gray-50 dark:bg-ink-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-accent text-gray-900 dark:text-white" />
                </div>
                <div className="space-y-1.5">
                    <label className="text-[10px] font-medium uppercase tracking-widest text-gray-400">Email</label>
