    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between gap-3">
                <h1 className="text-3xl md:text-4xl font-black text-primary tracking-tighter">Newsletter</h1>
                {subscribers.length > 0 && (
                    <button onClick={exportCsv} className="flex items-center gap-2 bg-[#46AEE8] text-white px-4 py-2.5 rounded-xl font-bold text-sm"><Download className="w-4 h-4" /> Export CSV</button>
                )}
            </div>

            {isLoading ? <TableSkeleton rows={8} cols={2} /> : subscribers.length === 0 ? <p className="text-secondary">No subscribers yet.</p> : (
                <>
                <div className="overflow-x-auto rounded-2xl border border-primary/5 bg-white">
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-primary/5">
                            {pageItems.map((s) => (
                                <tr key={s.id} className="text-primary">
