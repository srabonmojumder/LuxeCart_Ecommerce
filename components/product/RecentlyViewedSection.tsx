...
export default function RecentlyViewedSection({ excludeId, title = 'Recently Viewed' }: RecentlyViewedSectionProps) {
    ...
    return (
        <section className="py-12 md:py-20 bg-gray-50 dark:bg-slate-900/40">
            <div className="max-w-7xl mx-auto px-4 md:px-8">
                <div className="flex flex-wrap items-end justify-between gap-3 mb-10">
                    <div className="space-y-2">
                        <span className="inline-flex items-center gap-2 text-accent font-black tracking-[0.3em] text-[10px] md:text-xs uppercase">
                            <Clock className="w-3.5 h-3.5" /> Picked up where you left off
                        </span>
                        <h2 className="text-3xl md:text-5xl font-black text-primary dark:text-white tracking-tighter">{title}</h2>
                    </div>
                    <div className="flex items-center gap-4">
                        <button
                            onClick={clear}
                            className="text-[10px] font-black uppercase tracking-widest text-secondary dark:text-gray-400 hover:text-hot transition-colors"
                        >
                            Clear
                        </button>
                        <Link
